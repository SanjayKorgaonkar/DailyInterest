...
import { BadgeCheck, Calculator, Check, CheckCircle2, CornerDownRight, FileWarning, Landmark, Pencil, Trash2, Upload, UploadCloud, X } from "lucide-react";
...
export default function CCWorking({ facilities, month, onAction, refreshAll, focusFacilityId, autoOpenForm }) {
  ...
  const startEdit = (r) => { setEditingId(r.id); setEditForm({ date: r.date, value_date: r.value_date, debit: r.debit || "", credit: r.credit || "", bank_interest: r.bank ?? "" }); };
  const cancelEdit = () => { setEditingId(null); setEditForm({}); };
  const saveEdit = async (id) => {
    setSavingEdit(true);
    try {
      await api.put(`/cc-transactions/${id}`, { date: editForm.date, value_date: editForm.value_date, debit: Number(editForm.debit) || 0, credit: Number(editForm.credit) || 0, bank_interest: editForm.bank_interest === "" ? null : Number(editForm.bank_interest) });
      onAction("Transaction updated · interest recalculated");
      cancelEdit();
      load();
      refreshAll();
      ...
      <div className="table-wrap">
        ...
        <table>
          ...
          <tbody>
            ...
            {rows.map((r, i) => {
              const editable = !r.segment && !r.opening;
              const isEditing = editable && editingId === r.id;
              return (
                <tr key={`${r.id}-${i}`} className={r.segment ? "segment-row" : isEditing ? "editing-row" : ""} data-testid={r.segment ? "cc-segment-row" : "cc-row"}>
                  <td>{isEditing ? <input type="date" className="inline-edit-input" data-testid={`cc-edit-date-${r.id}`} value={editForm.date} onChange={(e) => setEditForm({ ...editForm, date: e.target.value })} /> : r.segment ? <span className="seg-label"><CornerDownRight size={12} /> Rate change</span> : r.opening ? <span className="seg-label">Opening balance</span> : fmtDate(r.date)}</td>
                  <td>{isEditing ? <input type="date" className="inline-edit-input" data-testid={`cc-edit-value-date-${r.id}`} value={editForm.value_date} onChange={(e) => setEditForm({ ...editForm, value_date: e.target.value })} /> : fmtDate(r.value_date)}</td>
                  <td>{fmtDate(r.to)}</td>
                  <td className="mono">{isEditing ? <input type="number" min="0" step="0.01" className="inline-edit-input mono" data-testid={`cc-edit-debit-${r.id}`} value={editForm.debit} onChange={(e) => setEditForm({ ...editForm, debit: e.target.value })} /> : (r.debit ? rupee(r.debit) : "—")}</td>
                  <td className="mono positive">{isEditing ? <input type="number" min="0" step="0.01" className="inline-edit-input mono" data-testid={`cc-edit-credit-${r.id}`} value={editForm.credit} onChange={(e) => setEditForm({ ...editForm, credit: e.target.value })} /> : (r.credit ? rupee(r.credit) : "—")}</td>
                  ...
                  <td className="mono">{isEditing ? <input type="number" min="0" step="0.01" className="inline-edit-input mono" data-testid={`cc-edit-bank-interest-${r.id}`} value={editForm.bank_interest} onChange={(e) => setEditForm({ ...editForm, bank_interest: e.target.value })} /> : (r.bank == null ? "—" : <>{rupee(r.bank)}{r.auto_matched && <span className="auto-badge" data-testid={`cc-auto-matched-${r.id}`}><BadgeCheck size={10} /> Auto</span>}</>)}</td>
                  <td className={`mono ${r.difference > 0 ? "danger-text" : "positive"}`}>{r.difference == null ? "—" : `${r.difference > 0 ? "+" : ""}${rupee(r.difference)}`}</td>
                  <td>
                    {isEditing ? (
                      <div className="row-edit-actions">
                        <button className="icon-btn" data-testid={`save-cc-edit-${r.id}`} aria-label="Save" disabled={savingEdit} onClick={() => saveEdit(r.id)}><Check size={14} /></button>
                        <button className="icon-btn" data-testid={`cancel-cc-edit-${r.id}`} aria-label="Cancel" onClick={cancelEdit}><X size={14} /></button>
                      </div>
                    ) : editable && (
                      <div className="row-edit-actions">
                        <button className="icon-btn" data-testid={`edit-cc-${r.id}`} aria-label="Edit" onClick={() => startEdit(r)}><Pencil size={13} /></button>
                        <button className="icon-btn" data-testid={`delete-cc-${r.id}`} aria-label="Delete" onClick={() => remove(r.id)}><Trash2 size={14} /></button>
                      </div>
                    )}
                  </td>
                </tr>
